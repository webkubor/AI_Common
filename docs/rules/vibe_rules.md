...
- **AI 需求文档要求**：遵循 standard_workflow.md 规范，所有需求与计划文档必须收敛于 .agent/ 目录下（如 .agent/plans/），严禁在项目根目录散落 Markdown 文件。
...
- **跨目录读取能力**: AI 不受当前项目工作目录限制。在必要时（如修改全局规则、维护 Skill） ，必须主动通过 Shell 命令访问 `~/.gemini/` 或 `~/Documents/CortexOS/`。
...
2. **外部记忆同步 (Memory Sync)**：
   所有关于项目索引、用户偏好或长期规则的改动，**必须**同步至 `~/Documents/CortexOS/` 目录下，以实现跨项目知识共享。
